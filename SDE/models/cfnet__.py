...
import math
from .monocular_way import UNet, UNet_P, UNet_KD, UNet_KD1, UNet_KD2
...
class cfnet(nn.Module):
    def __init__(self, maxdisp, use_concat_volume=False):
        ...
        Mish())
        '''
        self.bottle = nn.Sequential(convbn(64, 32, 3, 1, 1, 1),
                                 Mish(),
                                 nn.Conv2d(32, 1, kernel_size=1, padding=0, stride=1,
                                           bias=False))
        '''
        
        self.mono_way = UNet_KD2(n_channels = 32, n_classes = 1)
    ...
    def forward(self, left, right):
        left = left#self.norm(left)
        right = right#self.norm(right)
        
        features_left = self.feature_extraction(left)
        features_right = self.feature_extraction(right)
        
        
        mono_features = self.feature_extraction(right)
        
        
        #mono_features = self.bottle(mono_features["feature_last"])
        
        mono_depth = self.mono_up1(features_left["feature_last"])  
      
        mono_depth = self.mono_up2(mono_depth)  
 
        #mono_depth = self.mencoder(mono_features)
        #mono_depth = self.mdecoder(mono_depth)
        '''
        mono_depth = F.upsample(mono_depth, [256,512], mode='bilinear', 
                                    align_corners=True)#[("disp"), 0]
        '''
        #                           align_corners=True)#[("disp"), 0]

        mono_depth, lr_consist = self.mono_way(mono_depth)
        lr_consist = lr_consist.unsqueeze(1)

        lr_consist = F.upsample(lr_consist, [256,512], mode='bilinear', align_corners=True).squeeze(1)
        #print(mono_depth[("disp"), 3].size(), mono_depth[("disp"), 2].size())
    
        #mono_depth = F.upsample(mono_depth, [256,512], mode='bilinear', align_corners=True)#[("disp"), 0]
      
        #print(mono_depth.size())
        ...
        if self.training:
            ...
            result['monocular'] = mono_depth
            result['consist'] = lr_consist
            ...
            #pred_depth = F.upsample(mono_depth, [left.size()[2], left.size()[3]], mode='bilinear', align_corners=True)
            #pred_depth = torch.squeeze(pred_depth, 1)
